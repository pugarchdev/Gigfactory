...
const SimpleSection = ({ title, children, number }) => (
    <div className="group border-b border-zinc-200 dark:border-zinc-800 py-12 md:py-16 last:border-0">
        <div className="flex flex-col md:flex-row gap-8 md:gap-16">
            <div className="md:w-1/4 shrink-0">
                <span className="text-[#6EDD4D] font-mono text-sm tracking-widest mb-4 block">SECTION 0{number}</span>
                <h2 className="text-xl md:text-2xl font-bold text-zinc-900 dark:text-white group-hover:text-[#6EDD4D] transition-colors">{title}</h2>
            </div>
            <div className="md:w-3/4 text-zinc-600 dark:text-zinc-400 text-base md:text-lg leading-relaxed space-y-4">
                {children}
            </div>
        </div>
    </div>
...
export default function TermsPage() {
  return (
    <main className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 selection:bg-[#6EDD4D]/30 pb-32">
      ...
      <div className="container mx-auto px-6 pt-24 md:pt-40 max-w-6xl">
        <AnimatedSection animationClass="opacity-0 translate-y-10">
          <header className="mb-20 border-b border-zinc-200 dark:border-zinc-800 pb-12">
            ...
            <div className="flex items-center gap-6">
                <p className="text-zinc-600 dark:text-zinc-500 font-medium uppercase text-xs tracking-widest">Version 1.2</p>
                <div className="h-4 w-px bg-zinc-300 dark:bg-zinc-800"></div>
                <p className="text-zinc-600 dark:text-zinc-500 font-medium uppercase text-xs tracking-widest">Updated: 22 March 2023</p>
            </div>
          </header>
        </AnimatedSection>
        ...
        <div className="space-y-4">
            ...
            <SimpleSection title="Platform Access" number={2}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
                  {['Use a strong password', 'Do not share your account', 'Report breaches in 48h'].map(text => (
                      <div key={text} className="flex items-center gap-3 text-sm text-zinc-700 dark:text-zinc-300">
            ...
            <SimpleSection title="Usage Policy" number={4}>
              ...
              <ul className="space-y-3 list-none">
                  {['Illegal or harmful activity', 'Identity impersonation', 'Spam or misleading content', 'Unauthorized system access'].map(item => (
                      <li key={item} className="flex items-center gap-4 text-zinc-600 dark:text-zinc-400">
                          <span className="w-1.5 h-1.5 rounded-full bg-zinc-400 dark:bg-zinc-700"></span> {item}
                      </li>
                  ))}
              </ul>
            </SimpleSection>
          ...
          <AnimatedSection animationClass="opacity-0 translate-y-8" delay={300}>
            <SimpleSection title="Payments & Fees" number={7}>
              ...
              <div className="bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 rounded-2xl mt-4 inline-block">
                  <span className="text-xs text-zinc-600 dark:text-zinc-500 font-bold uppercase tracking-widest block mb-2">Subscription Cost</span>
                  <div className="text-3xl font-black text-zinc-900 dark:text-white">₹5999 <span className="text-base text-[#6EDD4D]">+ GST</span></div>
                  <p className="text-xs text-zinc-600 dark:text-zinc-500 mt-2 font-medium italic">Billed Quarterly (Once every 3 months)</p>
              </div>
            </SimpleSection>
          </AnimatedSection>
          ...
          <AnimatedSection animationClass="opacity-0 translate-y-8" delay={350}>
            <SimpleSection title="Refunds & Cancellation" number={8}>
              <p className="text-zinc-900 dark:text-white font-bold">All transactions are final. No refunds will be provided under any circumstances.</p>
        ...
        <AnimatedSection animationClass="opacity-0 translate-y-10" delay={500}>
            <div className="mt-32 pt-12 border-t border-zinc-200 dark:border-zinc-800 text-center">
                <p className="text-zinc-600 dark:text-zinc-500 text-sm">© {new Date().getFullYear()} Gigfactory Private Limited. All Rights Reserved.</p>
            </div>
        </AnimatedSection>
